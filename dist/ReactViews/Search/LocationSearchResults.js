...
const LocationSearchResults = observer(createReactClass({
    ...
    render() {
        ...
        const validResults = isDefined(locationSearchBoundingBox)
            ...
            : search.results;
        const results = validResults.length > MAX_RESULTS_BEFORE_TRUNCATING
            ? isExpanded
                ? validResults
                : validResults.slice(0, MAX_RESULTS_BEFORE_TRUNCATING)
            : validResults;
        return (React.createElement("div", { key: searchProvider.name, className: classNames(Styles.providerResult, {
                [Styles.isOpen]: this.state.isOpen,
                [Styles.dark]: this.props.theme === "dark",
                [Styles.light]: this.props.theme === "light"
            }) },
            React.createElement(RawButtonAndHighlight, { type: "button", fullWidth: true, onClick: this.toggleIsOpen },
                React.createElement(BoxSpan, { paddedRatio: 2, paddedVertically: 3, centered: true, justifySpaceBetween: true },
                    React.createElement(TextSpan, { textDarker: true, uppercase: true }, `${search.searchProvider.name} (${validResults === null || validResults === void 0 ? void 0 : validResults.length})`),
                    React.createElement(StyledIcon, { styledWidth: "9px", glyph: isOpen ? Icon.GLYPHS.opened : Icon.GLYPHS.closed }))),
            React.createElement(Text, { textDarker: true },
                React.createElement(SearchHeader, { searchResults: search, isWaitingForSearchToStart: this.props.isWaitingForSearchToStart }),
