...
const CollapsedNavigationPanel = observer((props) => {
    ...
    return (React.createElement(CollapsedNavigationBox, { column: true },
        ...
        React.createElement(ButtonsBox, null, items.map((item) => (React.createElement(NavigationButton, { key: item.id, title: applyTranslationIfExists(item.name, i18n), onClick: () => {
...
const CollapsedNavigation = observer(() => {
    const viewState = useViewState();
    useEffect(() => autorun(() => {
    ...
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement(CollapsedNavigationPanel, { items: items })));
});
