...
let MapNavigationItem = class MapNavigationItem extends React.Component {
    ...
    render() {
        ...
        return (React.createElement(Control, { ref: item.controller.itemRef },
            React.createElement(MapIconButton, { expandInPlace: expandInPlace === undefined ? true : expandInPlace, noExpand: item.noExpand, iconElement: () => React.createElement(Icon, { glyph: item.controller.glyph }), title: applyTranslationIfExists(item.title || item.name, i18n), onClick: () => {
                    item.controller.handleClick();
                }, disabled: item.controller.disabled, primary: item.controller.active, closeIconElement: closeTool ? () => React.createElement(Icon, { glyph: GLYPHS.closeTool }) : undefined }, applyTranslationIfExists(item.name, i18n))));
    }
};
