...
GnafAddressGeocoder.prototype.bulkConvertAddresses = function (tableStructure, corsProxy) {
    ...
    var suburbs = tableStructure.getColumnWithName(i18next.t("map.gnafAddressGeocoder.suburb"));
    smooshColumn(addressesCol, suburbs);
    var state = tableStructure.getColumnWithName(i18next.t("map.gnafAddressGeocoder.state"));
    smooshColumn(addressesCol, state);
    var postCodeCol = tableStructure.getColumnWithName(i18next.t("map.gnafAddressGeocoder.postcode"));
    ...
    return gnafApi
        ...
        that.tableStructure.addColumn(i18next.t("map.gnafAddressGeocoder.postcode"), matchedAddresses);
        that.tableStructure.addColumn(i18next.t("map.gnafAddressGeocoder.lon"), longValues);
        that.tableStructure.addColumn(i18next.t("map.gnafAddressGeocoder.lat"), latValues);
        that.tableStructure.addColumn(i18next.t("map.gnafAddressGeocoder.score"), resultScores);
        var addressGeocoderData = new BulkAddressGeocoderResult(that.startTime, that.numberOfAddressesConverted, addressesPlusInd.nullAddresses, missingAddresses);
...
//# sourceMappingURL=GnafAddressGeocoder.js.map