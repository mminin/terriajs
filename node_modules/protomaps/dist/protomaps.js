var protomaps = (() => {
  ...
  var queryLeafdir = (view, z2, x, y) => {
    const offset_len = queryView(view, z2 | 128, x, y);
    if (offset_len) {
      return {
        z: z2,
        x,
  ...
  var queryTile = (view, z2, x, y) => {
    const offset_len = queryView(view, z2, x, y);
    if (offset_len) {
      return {
        z: z2,
        x,
  ...
  var queryView = (view, z2, x, y) => {
    let m = 0;
    let n2 = view.byteLength / 17 - 1;
    while (m <= n2) {
      const k = n2 + m >> 1;
      const cmp = compare(z2, x, y, view, k * 17);
      if (cmp > 0) {
        m = k + 1;
      } else if (cmp < 0) {
        n2 = k - 1;
  ...
  var sortDir = (dataview) => {
    const entries = [];
    for (let i2 = 0; i2 < dataview.byteLength / 17; i2++) {
      entries.push(parseEntry(dataview, i2));
    }
  ...
  var PMTiles = class {
    ...
    fetchRoot(url) {
      return __async2(this, null, function* () {
        ...
        const header = parseHeader(new DataView(a2, 0, 10));
        let root_dir = new DataView(a2, 10 + header.json_size, 17 * header.root_entries);
        if (header.version === 1) {
    ...
    getZxy(z2, x, y) {
      return __async2(this, null, function* () {
        const root = yield this.getRoot();
        const entry = queryTile(root.dir, z2, x, y);
        if (entry)
          return entry;
        const leafcoords = deriveLeaf(root, { z: z2, x, y });
        if (leafcoords) {
          ...
          if (leafdir_entry) {
            ...
            return queryTile(new DataView(leafdir), z2, x, y);
          }
        }
        return null;
      });
      ...
      return __async(this, null, function* () {
        ...
        return new Promise((resolve, reject) => {
          if (result) {
            ...
            reject(new Error(`Tile ${c2.z} ${c2.x} ${c2.y} not found in archive`));
          }
        });
      });
    }
  ...
  var Static = class {
    ...
    drawContextBounds(ctx, top_left, bottom_right, width, height) {
      return __async(this, null, function* () {
        ...
        let center = new import_point_geometry7.default((top_left.y + bottom_right.y) / 2, (top_left.x + bottom_right.x) / 2);
